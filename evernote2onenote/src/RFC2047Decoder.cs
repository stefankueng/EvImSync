...
namespace Evernote2Onenote
{
    public class Rfc2047Decoder
    {
                    ...
                    case '=':
                        ...
                        if (!readingWord && peekAhead == '?')
                        {
                            if (!hasSeenAtLeastOneWord
                                || (currentSurroundingText.ToString().Trim().Length > 0))
                            {
        ...
        public static string ParseQuotedPrintable(Encoding enc, string input, int startPos = 0, bool skipQuestionEquals = false)
        {
